...
class SMSClient:
    """Send SMS messages via ClickSend or Twilio depending on configuration."""

    def __init__(self) -> None:
        self.provider = cfg.sms_provider

    def send(self, to: str, message: str) -> dict:
        """Send ``message`` to ``to`` using the configured provider."""
        if self.provider == "twilio":
            return self._twilio(to, message)
        return self._clicksend(to, message)

    def _clicksend(self, to: str, message: str) -> dict:
        auth = base64.b64encode(
            f"{cfg.clicksend_user}:{cfg.clicksend_key}".encode()
        ).decode()
    ...
    def _twilio(self, to: str, message: str) -> dict:
