...
class OSRMProvider(RouteProvider):
    def optimize(self, addresses: list[str]):
        ...
        resp = requests.get(f"https://router.project-osrm.org/trip/v1/driving/{coord_str}", params=params)
...
class MapboxProvider(RouteProvider):
    ...
    def optimize(self, addresses: list[str]):
        ...
        params = {"source": "first", "destination": "last", "roundtrip": "false", "access_token": self.token}
